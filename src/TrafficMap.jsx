...
export default function TrafficMap({network,route,layer,focus,junctions=[],hero=false}){
 ...
 },[network,route,layer,junctions.length]);
 ...
 useEffect(()=>{vehicleMarker.current?.setLatLng(positionAlong(metrics,progress));},[metrics,progress]);
 useEffect(()=>{if(!moving||!route)return;let handle;const start=performance.now(),base=progress;
  const tick=now=>{const value=Math.min(1,base+(now-start)/30000);setProgress(value);if(value>=1)setMoving(false);else handle=requestAnimationFrame(tick)};
